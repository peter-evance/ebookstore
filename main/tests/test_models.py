...
class TestModels(TestCase):
    '''New tests concept with factories fake data generation'''
    def test_active_manager_works(self):
        factories.BookFactory.create_batch(3, active=True)
        self.assertEqual(len(models.Book.objects.active()), 3)
    
    def test_create_order_works(self):
        bk1 = factories.BookFactory()
        bk2 = factories.BookFactory()
        ...
        with self.assertLogs("main.models", level="INFO") as cm:
            ...
            self.assertEquals(order.billing_address, billing.address)
            self.assertEquals(order.shipping_address, shipping.address)
